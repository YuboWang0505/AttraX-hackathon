...
- **消息乱序保护**：服务端为每条 chat 消息分配单调递增的 `seq_id`；M 端（以及 S 自己的回显）只在 `seq_id > last_applied_seq_id` 时应用该条消息的 intensity，**过时的 Gemini 回包被丢弃**。场景：S 快速连发两条，第二条命中码表秒回、第一条等 Gemini 1s 后回来 —— 第一条的 intensity 会被丢弃，避免"老意图覆盖新档位"
...
- **无 API Key 降级**：若 `GEMINI_API_KEY` 未配置，直接跳过第 2 层；第 1 层未命中的消息统一返回 `{intent_code: "S_TEASE_LIGHT", intensity: 1}`。开发期 / 黑客松前未拿到 key 时，全链路仍可跑通 demo
...
| 工程结构 | npm workspaces monorepo | 根目录管理 `frontend` / `backend` / `shared` 三个 package；`shared` 发布包名 `@attrax/shared` |
| 部署 | 本地 Node 进程 + ngrok（或局域网 demo） | 现场展示；ngrok 自动签 HTTPS/WSS 满足 Web Bluetooth 安全上下文要求 |
...
│   ├── package.json            <- deps: "@attrax/shared": "*"
│   ├── src/
│   │   ├── pages/
│   │   │   ├── Login.tsx
│   │   │   └── Chat.tsx
...
│   │   │   └── BluetoothStatus.tsx
│   │   ├── lib/
│   │   │   ├── ws.ts           <- WebSocket 客户端封装（seq_id 过滤在这层）
...
│   │   ├── rooms.ts            <- 房间匹配、断线重连窗口、seq_id 生成
│   │   ├── intent/
│   │   │   ├── pipeline.ts     <- 意图管道（0 层安全词 / 1 层码表 / 2 层 Gemini）
│   │   │   ├── keywords.ts     <- 第 1 层子串匹配
│   │   │   ├── gemini.ts       <- Gemini 调用层（含无 key 降级）
│   │   │   └── table.ts        <- 意图码表数据（附录 A）
│   │   └── safe-word.ts        <- 安全词归一化 + 相等匹配
│   ├── .env.example            <- GEMINI_API_KEY= (可留空)
│   └── tsconfig.json
└── shared/                     <- npm workspace, name: @attrax/shared
    ├── package.json
    ├── src/
    │   └── protocol.ts         <- WS 消息类型定义（前后端 import from "@attrax/shared"）
    └── tsconfig.json
```
...
| 项 | 状态 | 负责人 | 备注 |
|---|---|---|---|
| Gemini API Key | 待提供，已实现无 key 降级 | Vicky | 未配置时第 2 层直接回退 intensity=1；有 key 后塞 `backend/.env` 的 `GEMINI_API_KEY` 即生效 |
| 蓝牙协议（Service/Char UUID） | 阻塞，等待硬件组 | 硬件组 | 前端 `bluetooth.ts` 先留常量 TODO 占位，硬件固件就绪后改两处常量即可接入 |
| 硬件固件 | 阻塞，等待硬件组 | 硬件组 | v1 协议已确认：接收单字节 intensity（0x00 / 0x01 / 0x02 / 0x03），Write Without Response |
| UI 设计稿 | 无，按默认极简走 | - | 开发使用 PRD 5.1 风格规范，后续若有稿再调整 |
...
type ServerMsg =
  ...
  | { type: "room_waiting" }
  | { type: "chat"; from: "s" | "m"; text: string; intensity: 0 | 1 | 2 | 3; intent_code: string; seq_id: number; timestamp: number /* ms */ }
